    ...
    cy.get("tbody tr").each(($el, index) => {
      if (index > 0) {
        cy.get($el)
          ...
          .invoke("text")
          .then((nome) => {
            if (index == 1) expect(nome).to.be.equal("Ração");

            if (index == 2) expect(nome).to.be.equal("Petiscos");

            if (index == 3) expect(nome).to.be.equal("Coleira");

            if (index == 4) expect(nome).to.be.equal("Remédios");
          });
        ...
        cy.get($el)
          ...
          .invoke("text")
          .then((valor) => {
            if (index == 1) expect(valor).to.be.equal("1000");

            if (index == 2) expect(valor).to.be.equal("30");

            if (index == 3) expect(valor).to.be.equal("150");

            if (index == 4) expect(valor).to.be.equal("1");
          });
      }
    });
